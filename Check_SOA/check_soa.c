...
/* Our own routines; code include later */
...
int *nsNum;
{
	 ...
	 * algorithms, we would have used res_search() instead.
	 */
	if ((responseLen =
	 ...
	 * NS recrods may be in the answer section and/or in the
...
/*********************************************************************************************************
 * addNameServers -- Look at the resource records from a section.
 * 		Save the names of all name servers.
 *
*********************************************************************************************************/
void
 ...
 * 		for the SOA record fo the given zone. Report any
 * 		errors encountered (e.g., a name server not running or
 * 		the response not being an authoritative response).
 * 		If there are  no errors, print out the serial number for the zone.
 *
*********************************************************************************************************/
void
	...
	for (nsNum--; nsNum > 0; nsNum--) {
		...
		if((responseLen = res_send((u_char *)&query, 	/* the query */
			...
			if (errno == ECONNREFUSED) { /* no server on the host */
				(void) fprintf(stderr, "There is no name server running on %s\n", nsList[nsNum]);
			}
			else {						/* anything else: no response */
				(void) fprintf(stderr, "There was no response from %s\n", nsList[nsNum]);
			}
			continue; /* nsNum for-loop */
		}
		 ...
		 * authoritative, report it, and go on tho the next server.
		 */
		if (!ns_msg_getflag(handle, ns_f_aa)) {
			(void) fprintf(stderr, "%s is not authoritative for %s\n", nsList[nsNum], domain);
			continue; /* nsNum for-loop */
		}
		...
		if(ns_msg_count(handle, ns_s_an) != 1) {
			(void) fprintf(stderr, "%s: expected 1 answer, got %d\n", nsList[nsNum], ns_msg_count(handle, ns_s_an));
			continue; /* nsNum for-loop */
		}
		...
		if (ns_rr_type(rr) != ns_t_soa) {
			(void) fprintf(stderr, "%s: expected answer type %d, got %d\n", nsList[nsNum], ns_t_soa, ns_rr_type(rr));
			continue; /* nsNum for-loop */
		}

		/*
		 * Set cp to point to the SOA record.
		 */
